...
const MODE_LABELS = { contact: "Generate Contact QR", basic: "Generate Basic QR", trackable: "Generate Trackable QR" };
...
function buildVCard() {
  ...
  return [
    ...
    tel ? `TEL;TYPE=CELL:${escapeVCard(tel)}` : "",
...
function makeProbeQr(text, level) {
  const holder = document.createElement("div");
  return new QRCode(holder, { text, width: 256, height: 256, correctLevel: level });
}

function versionFromModules(modules) {
  return Math.round((modules - 17) / 4);
}

function dimensions(requestedPx, moduleCount) {
  const pitches = moduleCount + 8;
  const unit = Math.max(1, Math.floor(requestedPx / pitches));
  return { unit, corePx: moduleCount * unit, quietPx: 4 * unit, totalPx: pitches * unit };
}
...
function renderSquare(payload, level, requestedPx, moduleCount) {
  ...
  outer.width = d.totalPx;
  outer.height = d.totalPx;
  ...
  if (src.tagName === "CANVAS") {
    ctx.drawImage(src, d.quietPx, d.quietPx, d.corePx, d.corePx);
  } else {
    const img = new Image();
    img.onload = () => ctx.drawImage(img, d.quietPx, d.quietPx, d.corePx, d.corePx);
    img.src = src.src;
  }
...
function renderCircle(payload, levelKey, requestedPx, moduleCount) {
  ...
  latestCircleQr = new QRCodeStyling({
    ...
    data: payload,
    margin: 0,
    qrOptions: { errorCorrectionLevel: levelKey },
...
function updateDiagnostics(payload, levelKey, moduleCount) {
  ...
  const physical = Number(els.physicalSize.value);
  const physicalUnit = els.physicalUnit.value;
  ...
  if (physical > 0) {
    ...
    if (moduleMm >= 0.5) {
      els.scanStatus.className = "status good";
      els.scanStatus.textContent = "Good: module size is generous for many physical reproduction methods. Still test the finished artifact before batch production.";
    } else if (moduleMm >= 0.4) {
      els.scanStatus.className = "status warn";
      els.scanStatus.textContent = "Caution: this is relatively dense at the selected physical size. Test carefully on the final material and consider enlarging it or reducing payload.";
    } else {
      els.scanStatus.className = "status risk";
      els.scanStatus.textContent = "High density: consider increasing physical size, shortening the payload, or lowering error correction before physical production.";
    }
  } else {
    els.diagPhysical.textContent = "Not entered";
    els.diagModule.textContent = "Not calculated";
    els.scanStatus.className = "status info";
    els.scanStatus.textContent = "Enter an intended physical size to estimate physical module width.";
  }
}
...
function clearError() {
  setError("");
  els.qrError.textContent = "";
  els.qrError.classList.add("hidden");
}

function showQrError(message) {
  els.qrError.textContent = message;
  els.qrError.classList.remove("hidden");
}

function showResult(payload, trackingMeta = null) {
  const levelKey = els.ecLevel.value;
  const level = EC_MAP[levelKey] || EC.M;
  const requestedPx = parseInt(els.imageSize.value, 10) || 320;
  const probe = makeProbeQr(payload, level);
  const moduleCount = probe._oQRCode.getModuleCount();

  if (els.qrShape.value === "circle") renderCircle(payload, levelKey, requestedPx, moduleCount);
  else renderSquare(payload, level, requestedPx, moduleCount);

  latestPayload = payload;
  latestModuleCount = moduleCount;
  latestTrackingMeta = trackingMeta;
  ...
  if (trackingMeta) {
    ...
    els.trackableMeta.innerHTML = `<strong>Trackable QR created</strong><br>Tracking ID: ${safe(trackingMeta.trackingId)}<br>Tracking URL: ${safe(trackingMeta.trackingUrl)}<br>Notification email: ${safe(trackingMeta.notificationEmail || "default / none") }<br>Mode: ${safe(trackingMeta.redirectMode)}`;
    els.trackableMeta.classList.remove("hidden");
  } else {
    els.trackableMeta.innerHTML = "";
    els.trackableMeta.classList.add("hidden");
  }
}
...
async function generateTrackablePayload() {
  ...
  const body = new URLSearchParams({
    ...
    label: els.trackableLabel.value.trim(),
    destinationUrl,
    notificationEmail: els.notificationEmail.value.trim(),
    redirectMode: els.trackingMode.value
  });
...
async function generate() {
  ...
  try {
    if (activeMode === "contact") {
      showResult(buildVCard());
    } else if (activeMode === "basic") {
      const text = els.basicText.value.trim();
      if (!text) throw new Error("Enter a URL or text for the Basic QR.");
      showResult(text);
    } else {
      const { payload, meta } = await generateTrackablePayload();
      showResult(payload, meta);
    }
  } catch (error) {
    showQrError(`Could not generate the QR. ${error && error.message ? error.message : String(error)}`);
  } finally {
    els.generateBtn.disabled = false;
    els.generateBtn.textContent = MODE_LABELS[activeMode] || originalText;
  }
}
...
async function downloadCircle(filename) {
  ...
  const requestedPx = parseInt(els.imageSize.value, 10) || 320;
  const d = dimensions(requestedPx, latestModuleCount);
  const blob = await latestCircleQr.getRawData("png");
  if (!blob) return;
  const objectUrl = URL.createObjectURL(blob);
  const img = new Image();
  img.onload = () => {
    const canvas = document.createElement("canvas");
    canvas.width = d.totalPx;
    canvas.height = d.totalPx;
    ...
    ctx.fill();
    ctx.drawImage(img, d.quietPx, d.quietPx, d.corePx, d.corePx);
...
function filenameForMode() {
  return `${activeMode}-qr-${els.qrShape.value}.png`;
}

async function downloadCurrent() {
  if (!latestPayload) {
    showQrError("Generate a QR before downloading it.");
    return;
  }
  if (els.qrShape.value === "circle") await downloadCircle(filenameForMode());
  else downloadCanvas(latestCanvas, filenameForMode());
}

function regenerateDirectIfPossible() {
  if (!latestPayload || activeMode === "trackable") return;
  try {
    if (activeMode === "contact") showResult(buildVCard());
    if (activeMode === "basic" && els.basicText.value.trim()) showResult(els.basicText.value.trim());
  } catch (_) {}
}
...
[els.qrShape, els.ecLevel, els.imageSize, els.physicalSize, els.physicalUnit].forEach(el => el.addEventListener("change", () => {
  if (latestPayload && activeMode === "trackable") {
    try { showResult(latestPayload, latestTrackingMeta); } catch (_) {}
